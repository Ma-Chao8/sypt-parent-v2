...
jQuery.extend({
	...
	createUploadIframe : function(id, uri) {
		...
		if (window.ActiveXObject) {
			io = document.createElement('<iframe id="' + frameId + '" name="'
					+ frameId + '" />');
	...
	createUploadForm : function(id, fileElementId) {
		...
		for(i in fileElementId){
			var fileId = 'jUploadFile' + fileElementId[i];
	...
	uploadHttpData : function(r, type) {
		...
		if (type == "json") {
			//eval("data = " + data);
			console.log(data);
			var str = data.replace('<pre style="word-wrap: break-word; white-space: pre-wrap;">',"").replace("</pre>","");  
			data = $.parseJSON(str);
		}
