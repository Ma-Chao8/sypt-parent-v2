...
(function (w, $, m, wx) {
    ...
    $("#selectAgent").click(function () {
        //请求经销商数据
        $("#agentPopover input[name=agent_name]").val("");
        m('#agentPopover').popover('show');//show hide toggle
        getAgents(true)
    });

    //条件搜索
    $("#agentPopover .search-form button[name=search]").click(function () {
        getAgents(true)
    });

    //加载更多经销商
    $("#agentPopover .load-more").click(function () {
        getAgents(false)
    });

    //选择经销商
    $(document).on("click", "#agentPopover ul li", function () {
        m('#agentPopover').popover('hide');//show hide toggle
        var agent_id = $(this).find("a").attr("data");
        var agent_name = $(this).find("a").text();
        $("#selectAgent").attr("data", agent_id ? agent_id : "");
        $("#selectAgent").text(agent_name ? agent_name : "选择经销商");
    });
     ...
     */
    function getAgents(isRefresh) {
        var agent_name = $("#agentPopover input[name=agent_name]").val();
        if (!agent_name) {
            agent_name = "";
        }
        if (isRefresh) {
            m("#agentPopover .mui-scroll-wrapper").scroll().scrollTo(0, 0);
            agentPage = 1;
            $("#agentPopover ul").empty();
        }

        $.ajax({
            url: w.ctxPath + "agent/page",
            type: "GET",
            dataType: "JSON",
            data: {start: agentPage, pageSize:pageSize, agentName: agent_name},
            success: function (result) {
                if (result.success) {
                    ...
                    for (var index in items) {
                        ...
                        $("#agentPopover ul").append('<li class="mui-table-view-cell"><a data="' + item.agentId + '">' + item.agentName + '</a></li>')
                    }
                } else {
    ...
    function submitData() {
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    m.toast(result.msg, {duration: 'short', type: 'div'});
                    clearCodeList();
                } else {
                    m.alert(result.data);
                }
